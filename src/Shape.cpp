...
{
    // Load geometry
    loadObj(meshName, posBuf, norBuf, texBuf);
}

void Shape::refreshNormals()
{
    // auto-calculates normals of shape; used on simple manually-typed objs
    // only works with low-poly style shapes i think (without normal interpolation)
    int numFaces = posBuf.size() / 9;
    norBuf.clear();

    for (int i = 0; i < numFaces; i++) {
        glm::vec3 v1(posBuf[9 * i + 0], posBuf[9 * i + 1], posBuf[9 * i + 2]);
        glm::vec3 v2(posBuf[9 * i + 3], posBuf[9 * i + 4], posBuf[9 * i + 5]);
        glm::vec3 v3(posBuf[9 * i + 6], posBuf[9 * i + 7], posBuf[9 * i + 8]);

        glm::vec3 nor = normalize(cross(v2 - v1, v3 - v1));
        for (int j = 0; j < 3; j++) {
            norBuf.push_back(nor.x);
            norBuf.push_back(nor.y);
            norBuf.push_back(nor.z);
        }
    }
}
